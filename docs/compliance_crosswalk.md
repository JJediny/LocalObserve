...
| :--- | :--- | :--- | :--- | :--- |
| **Req-1** | Appendix B.3, B.5 (Structured Schema & Timestamp Sync) | JSON-structured log formatting across all system collectors, aligned with authoritative NTP synchronization (traceable to USNO/NIST). | [Issue Details](#requirement-1-centralized-structured-logging-for-all-components) | 🟡 Planned |
| **Req-2** | Appendix B.1 (Retention Policy: Search vs. Retrieval) | 6-month hot searchable data retention in OpenObserve alongside a 12-month cold retrieval policy. | [Issue Details](#requirement-2-log-retention-and-secure-archival-policy) | 🟡 Planned |
| **Req-3** | Par. 125-131, Appendix C (Access Controls & JIT) | SOC integration, Just-In-Time (JIT) access policies, and audited log exports for CISA/FBI. | [Issue Details](#requirement-3-access-controls-and-rbac-for-log--network-data) | 🟡 Planned |
| **Req-4** | Appendix A, B.5.b (Network Capture Minimization) | Integrated flow captures via `goflow2` with PII filters to protect sensitive identifiers. | [Issue Details](#requirement-4-network-capture-controls-minimization-and-retention) | 🟡 Planned |
| **Req-5** | Appendix C (Log Veracity & Hashing) | Hashing log batches for tamper-evidence and auditing configurations. | [Issue Details](#requirement-5-tamper-evident-audit-trail-for-logging-pipeline--config) | 🟡 Planned |
| **Req-6** | Appendix B.5.k, Appendix C (Pipeline Monitoring & Alerts) | Pipeline health monitoring, threshold alerting for data drops, and tuning alerts. | [Issue Details](#requirement-6-monitoring-alerting-and-incident-response-hooks-for-logging-anomalies) | 🟡 Planned |
| **Req-7** | Appendix C (Data Protection & KMS) | TLS 1.3 transit encryption, AES-256 rest encryption, and integration with KMS. | [Issue Details](#requirement-7-data-protection-for-stored-log--capture-artifacts-encryption--kms) | 🟡 Planned |
| **Req-8** | Appendix A, C (Compliance Validation & CI) | Automated CI regression tests verifying compliance controls and fields. | [Issue Details](#requirement-8-documentation-tests-and-ci-checks-for-compliance-controls) | 🟡 Planned |
...
- **Assigned Tracker**: *To be updated with issue link*
