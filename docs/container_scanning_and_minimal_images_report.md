...
> [!CAUTION]
> **Validation Status:** The CVE counts, image sizes, and reduction percentages in the
> table below are **target values** defined in `config/image_sourcing.yaml`, not
> scan-verified results. The referenced Dockerfiles (e.g.,
> `docker/Dockerfile.openobserve.distroless`) do not yet exist in the repository.
> To produce verified numbers, run `trivy image` and `osv-scanner` against built
> images and update this table with actual output. Treat all `0 CVE` entries as
> goals pending validation, not confirmed findings.

---

## 1. Container Scanning Benchmark Matrix

The following matrix compares **Upstream Publisher Vendor Images** against **Custom Minimal Distroless / Chainguard Dockerfiles** using target values from `config/image_sourcing.yaml`. These values must be validated with `trivy` and `osv-scanner` before relying on them for production decisions.
